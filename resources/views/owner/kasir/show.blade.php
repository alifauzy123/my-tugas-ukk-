...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2">
            ...
            <div class="p-8 space-y-6">
                ...
                <div class="border-b-2 border-gray-100 pb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
                        <div>
                            <p class="text-gray-500 text-xs font-semibold mb-1">Avatar</p>
                            @if(!empty($kasir->avatar))
                                <img src="{{ $kasir->avatar }}" alt="Avatar" class="w-16 h-16 rounded-full object-cover border">
                            @endif
                        </div>
                        <div>
                            <p class="text-gray-500 text-xs font-semibold mb-1">Bio</p>
                            <p class="text-gray-800">{{ $kasir->bio ?? '' }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500 text-xs font-semibold mb-1">Telepon Kantor</p>
                            <p class="text-gray-800">{{ $kasir->telepon_kantor ?? '' }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500 text-xs font-semibold mb-1">Nama Bank</p>
                            <p class="text-gray-800">{{ $kasir->nama_bank ?? '' }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500 text-xs font-semibold mb-1">Nomor Rekening</p>
                            <p class="text-gray-800">{{ $kasir->nomor_rekening ?? '' }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500 text-xs font-semibold mb-1">Atas Nama Rekening</p>
                            <p class="text-gray-800">{{ $kasir->atas_nama_rekening ?? '' }}</p>
                        </div>
                    </div>
                </div>
        ...
        <div class="mt-6 flex gap-4">
            <a href="{{ route('owner.kasir.edit', $kasir->id) }}" class="flex-1 px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg hover:from-blue-600 hover:to-blue-700 transition-all duration-200 font-semibold text-sm flex items-center justify-center gap-2 shadow-lg">
            ...
            <a href="{{ route('owner.kasir.index') }}" class="flex-1 px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-all duration-200 font-semibold text-sm flex items-center justify-center gap-2">
                <i class="fas fa-arrow-left"></i>
                Kembali
            </a>
        </div>
