...
<div class="bg-white w-full rounded-lg shadow-xl p-6 md:p-12 border-t-[6px] border-red-700 relative">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
        ...
        <div class="flex items-center gap-2" id="aksiButtons">
            <a href="{{ route('pelanggan.create') }}" class="bg-white border-2 border-red-700 text-red-700 px-4 py-2 rounded-md hover:bg-red-700 hover:text-white text-sm">
                + Create New
            </a>
    ...
    <table class="w-full text-sm border-2 border-gray-400 rounded">
        <thead>
            <tr class="bg-gray-100">
                ...
                <th class="px-4 py-2 border-2 border-gray-300">Email</th>
                <th class="px-4 py-2 border-2 border-gray-300">No Telepon</th>
                <th class="px-4 py-2 border-2 border-gray-300">Alamat</th>
        ...
        <tbody id="dataTable">
            ...
            <tr onclick="selectRow(this)" data-id="{{ $p->id }}" class="bg-white hover:bg-yellow-100 cursor-pointer">
                ...
                <td class="px-4 py-2 border-2 border-gray-200">{{ $p->kode_pelanggan }}</td>
                <td class="px-4 py-2 border-2 border-gray-200">{{ $p->nama_pelanggan }}</td>
                <td class="px-4 py-2 border-2 border-gray-200">{{ $p->email }}</td>
                <td class="px-4 py-2 border-2 border-gray-200">{{ $p->no_telp }}</td>
                <td class="px-4 py-2 border-2 border-gray-200">{{ $p->alamat }}</td>
...
<script>
    ...
    document.getElementById('btnRead').addEventListener('click', (e) => {
        e.stopPropagation();
        if (selectedId) window.location.href = `/pelanggan/${selectedId}`;
    });

    document.getElementById('btnEdit').addEventListener('click', (e) => {
        e.stopPropagation();
        if (selectedId) window.location.href = `/pelanggan/${selectedId}/edit`;
    });
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            form.method = 'POST';
            form.action = `/pelanggan/${selectedId}`;    
